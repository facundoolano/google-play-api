...
router.get('/apps/', async function (req, res, next) {
  ...
  try {
    ...
    for (let app of apps) {
      try {
        ...
        for (let app2 of apps2) {
          if (!arr.hasOwnProperty(app2.appId)) {
            arr[app2.appId] = app2;
          }
        }
      } catch (err) {
    ...
    if (count < total) {
      console.log(`Not all apps for term: ${opts.term} have been processed, calling searchTermMore again`);
      const remainingApps = apps.slice(count);
      const remainingResults = await searchTermMore(opts.term, remainingApps, arr);
      const appList = toList(Object.values(Object.assign({}, arr, remainingResults)));
      res.json(appList);
    } else {
      const appList = toList(Object.values(arr));
      res.json(appList);
    }
  } catch (error) {
    next(error);

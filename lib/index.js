...
const cleanUrls = (req) => (app) => Object.assign({}, app, {
  ...
  categories: buildUrl(req, 'categories/') + '?' + qs.stringify({ cat: app.genre })
});
...
*/
    ...
    function checkFlag() {
      if(count != total) {
         setTimeout(checkFlag, 1000);
          ...
          for (const app of Object.values(arr)) {
            file.write(JSON.stringify(app) + "\n")
            console.log(app)
          } 
          file.end()
      }
  ...
  gplay.search(opts)
   .then((apps) => apps.map(cleanUrls(searchTermMore(req))))
   .then(toList)
